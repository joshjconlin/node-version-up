...
module.exports = {
  ...
  // todo: get status, make sure branch is clean, fail out otherwise.
      ...
      if (answer === 'y') {
        ...
        if (changeAnswer === 'y') {
          ...
          return resolve(new Promise((resolve1, reject1) => {
            exec(`git checkout ${newBranch}`, error => {
              if (error) {
  ...
  getGitBranch() {
    return new Promise((resolve, reject) => {
      exec(`git rev-parse --abbrev-ref HEAD`, (error, value) => {
        if (error) {
          return reject(error);
        }

        console.log(value, 'branch value');
